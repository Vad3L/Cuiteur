...
/* ------------------------------------------------------------------------------
    ...
    - étape 1 : vérifier les parametre et les decrypter
...
FROM users LEFT OUTER JOIN estabonne on usID=eaIDAbonne AND eaIDUser = 5
WHERE usID={$id};";
...
vl_aff_debut('Abonnements | Cuiteur','../styles/cuiteur.css');
vl_aff_entete(false,'Les abonnements de '.em_html_proteger_sortie($T['usPseudo']));
 ...
 * @param int   $id est l'id de la personne choisis
 */
...
$S = "SELECT usID,usPseudo,usNom,eaIDUser,usAvecPhoto,eaIDAbonne
    ...
    WHERE usID IN (SELECT usID
                        FROM `estabonne` INNER JOIN users on usID = eaIDAbonne
                        WHERE eaIDUser = {$id});";
    $R = vl_bd_send_request($bd, $S);
    $TT;
